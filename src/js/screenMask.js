  ...
  mouseEvents = () => {
    ...
    let mousemoveListener = (event) => {
      ...
      if (clicked) {
        ...
        //// Position of element
        let right = startPosition.x < x ? startPosition.x : (x > innerWidth ? innerWidth - 5 : x);
        let left = x < 5 ? 5 : (x < startPosition.x ? x : startPosition.x);
        if (direction === 'rtl') {
          right = x < startPosition.x ? (innerWidth - startPosition.x) : (x < innerWidth ? innerWidth - x : 0);
          left = x < startPosition.x ? (innerWidth - x) : (innerWidth - startPosition.x);
        }
        console.log({right, left});
        console.log({startPosition: startPosition.x});
        console.log({x});
